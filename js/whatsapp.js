...
(function () {
  ...
  function buildMessage(productName) {
    if (productName && productName.trim()) {
      return "I want to order this product: " + productName.trim() + ".";
    }
    return DEFAULT_MESSAGE;
  }

  function buildUrl(productName) {
    return (
      "https://wa.me/" +
      PHONE +
      "?text=" +
      encodeURIComponent(buildMessage(productName))
    );
  }

  function setWhatsAppProduct(productName) {
    var link = document.getElementById("whatsappFloat");
    if (!link) return;
    link.href = buildUrl(productName);
    link.setAttribute(
      "aria-label",
      productName
  ...
  function injectWhatsAppButton() {
    ...
    link.innerHTML =
      '<span class="whatsapp-float-glow" aria-hidden="true"></span>' +
    ...
    document.addEventListener("click", function (e) {
      ...
      if (nameEl) setWhatsAppProduct(nameEl.textContent.trim());
    });
  }

  window.setWhatsAppProduct = setWhatsAppProduct;
